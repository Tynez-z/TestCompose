package com.example.testCompose.data.remote.api
...
interface ApiMovies {
    ...
    companion object {
        ...
        const val GET_MOVIE_VIDEO = "3/movie/{movie_id}/videos?language=en-US"
        ...
        const val GET_MOVIE_ACTORS = "3/movie/{movie_id}/credits"
    ...
    suspend fun getMovies(
        @Query(API_KEY) apiKey: String, @Query(PAGE) @IntRange(
            from = 1, to = MAX_PAGE_SIZE.toLong()
        ) pageNumber: Int = DEFAULT_PAGE_SIZE
    ): Response<MoviesResponse>

    @GET(GET_SIMILAR_FILMS)
    suspend fun getSimilarMovies(
        @Path(MOVIE_ID) movie_id: Int, @Query(API_KEY) apiKey: String, @Query(PAGE) @IntRange(
            from = 1, to = MAX_PAGE_SIMILAR_MOVIE.toLong()
        ) pageNumber: Int = DEFAULT_PAGE_SIZE
    ): Response<SimilarMovies>

    @GET(GET_MOVIE_ACTORS)
     fun getMovieActors(
    ...
    suspend fun getSearchMovie(
        @Query(API_KEY) api_key: String, @Query(QUERY) query: String, @Query(PAGE) @IntRange(
            from = 1, to = MAX_PAGE_SIMILAR_MOVIE.toLong()
        ) pageNumber: Int = DEFAULT_PAGE_SIZE
